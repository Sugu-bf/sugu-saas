...
// ── Icon mapping (same pattern as dashboard) ──
const WALLET_ICONS: Record<string, ReactNode> = {
...
/** Derive the transaction category from WalletEntry */
function getEntryCategory(entry: WalletEntry): string {
...
export default function VendorWalletPage() {
  const { data, isLoading, isError, error, refetch } = useVendorWallet();
  const [activePeriod, setActivePeriod] = useState("30d");
  const [activeFilter, setActiveFilter] = useState("all");

  if (isLoading) return <VendorWalletLoading />;

  if (isError) {
    return (
      <div className="flex min-h-[60vh] items-center justify-center">
        <ErrorState
          title="Erreur du portefeuille"
          description={
            error?.message ||
            "Impossible de charger le portefeuille. Veuillez réessayer."
          }
          onRetry={() => refetch()}
        />
      </div>
  ...
  const previousMonthRevenue = Math.round(totalRevenue * 0.889); // mock comparison
    ...
    <div className="mx-auto max-w-7xl space-y-4 lg:space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-5 lg:gap-4">
        ...
        <section
          ...
          aria-labelledby="wallet-chart-title"
        >
          <div className="flex items-center justify-between">
            <h2
              id="wallet-chart-title"
          ...
          <div className="mt-3 flex flex-col gap-1 px-1 sm:flex-row sm:justify-between lg:mt-4">
            ...
            <span className="text-xs text-gray-500">
              vs mois précédent:{" "}
              <strong className="text-gray-700">
                {formatCurrency(previousMonthRevenue)} FCFA
              </strong>{" "}
...
/** KPI Card with gradient background — same pattern as dashboard */
...
/** SVG Area Chart — pure CSS, no JS library (same as dashboard) */
function RevenueChart({ data }: { data: WalletRevenuePoint[] }) {
  if (data.length === 0) {
    return (
  ...
  return (
    <div className="relative">
      ...
      >
        <defs>
          <linearGradient id="wallet-chart-gradient" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#f15412" stopOpacity="0.3" />
            <stop offset="100%" stopColor="#f15412" stopOpacity="0.02" />
          </linearGradient>
          <linearGradient id="wallet-line-gradient" x1="0" y1="0" x2="1" y2="0">
        ...
        {/* Area fill */}
        <path d={areaPath} fill="url(#wallet-chart-gradient)" />

        {/* Line */}
        <path
          d={linePath}
          fill="none"
          stroke="url(#wallet-line-gradient)"
          strokeWidth="2.5"
          strokeLinecap="round"
...
/** Next Payout Card */
function NextPayoutCard({ payout }: { payout: NextPayout }) {
  return (
...
function TransactionRow({ entry }: { entry: WalletEntry }) {
  const category = getEntryCategory(entry);
  const config = ENTRY_TYPE_CONFIG[category] ?? ENTRY_TYPE_CONFIG.revenue;
  ...
  return (
    <div className="flex flex-col gap-2 border-b border-gray-100/60 px-2 py-3.5 transition-colors active:bg-white/40 lg:flex-row lg:items-center lg:gap-4 lg:hover:bg-white/40">
      ...
      <div className="hidden lg:flex lg:w-20 lg:justify-center">
        ...
        >
          {ENTRY_TYPE_CONFIG[category]?.label ?? "Revenu"}
        </span>
      </div>
      ...
      {/* Status */}
      <span
        className={`hidden text-xs font-medium lg:block lg:w-24 lg:text-right ${statusInfo.color}`}
      >
