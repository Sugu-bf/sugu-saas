...
export default function SuguBoxInboundPage() {
  const [shipments, setShipments] = useState<SuguBoxInboundShipment[]>([]);
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-3">
            ...
            <span className="px-2.5 py-0.5 rounded-full bg-green-100 text-green-600 border border-green-200 text-xs font-bold">
              {total}
            </span>
          </div>
          <p className="text-sm text-gray-500 mt-1 font-medium dark:text-gray-400">
            Produits envoyés à l&#39;entrepôt Sugu
          </p>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-gray-100 bg-white shadow-sm overflow-hidden dark:border-gray-800 dark:bg-gray-900">
              ...
              <tbody>
                {shipments.length === 0 ? (
                  <tr>
                    ...
                    >
                      <PackagePlus className="mx-auto h-12 w-12 text-gray-200 mb-3" />
                      Aucune réception
                    </td>
                  </tr>
                ) : (
                  shipments.map((s) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-3 text-center">
                        <span
                          className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-bold border ${STATUS_COLORS[s.status] ?? "bg-gray-100 text-gray-600"}`}
                        >
                          {s.status_label}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-3 text-center">
                        <Link
                          href={`/vendor/sugubox/inbound/${s.id}`}
        ...
        {lastPage > 1 && (
          <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between dark:border-gray-800">
            <p className="text-xs text-gray-500">{total} réceptions</p>
