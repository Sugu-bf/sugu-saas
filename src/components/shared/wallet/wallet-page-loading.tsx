interface WalletPageLoadingProps {
  ariaLabel: string;
  srMessage: string;
  /** "driver" shows a rounded icon placeholder in the header; "vendor" shows a simpler header */
  headerStyle: "driver" | "vendor";
}

/** Shared loading skeleton for both driver earnings and vendor wallet pages */
export function WalletPageLoading({
  ariaLabel,
  srMessage,
  headerStyle,
}: WalletPageLoadingProps) {
  return (
    <div
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        {headerStyle === "driver" ? (
          <div className="flex items-center gap-3">
            <div className="h-11 w-11 rounded-2xl bg-white/50" />
            <div>
              <div className="h-8 w-40 rounded-xl bg-white/50" />
              <div className="mt-2 h-4 w-56 rounded-lg bg-white/40" />
            </div>
          </div>
        ) : (
          <div>
