...
import { sendCourierTyping } from "../hooks";

import { useCourierConversations, useCourierConversation } from "../hooks";
...
/**
 * CourierMessagingPage — 3-panel layout (desktop) / navigation (mobile)
 *
 ...
 */
export function CourierMessagingPage() {
  ...
  const { data: conversationsData, isLoading: isLoadingConversations } =
    useCourierConversations();
  const { data: conversationDetail } = useCourierConversation(selectedId);
  ...
  const onMessageCreated = useCallback(
    (payload: MessageCreatedPayload) => {
      ...
      if (payload.conversation_id === selectedId) {
        qc.invalidateQueries({
          queryKey: queryKeys.driver.messages(selectedId),
        });
      }
    },
    [qc, selectedId],
